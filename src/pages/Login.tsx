...
function PasswordForm({
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      // Сетевой сбой раньше показывался как «не удалось войти» — и это читалось
      // как неверный пароль. Ксюша так потеряла время: пароль был правильный,
      // до сервера просто не доходил запрос. Разводим случаи явно.
      ...
      setError(
        status === 401
          ...
          : status === 429
            ...
            : status >= 500
              ? "Сервер не отвечает. Напишите Матвею — это не ваша ошибка."
              : "Не получается связаться с сервером. Проверьте интернет: "
                + "если сайт открывается, а вход не работает, попробуйте "
                + "мобильный интернет или другой браузер.",
      );
    } finally {
      setBusy(false);
    }
